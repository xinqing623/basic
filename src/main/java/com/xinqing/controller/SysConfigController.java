...
public class SysConfigController extends BaseController {
	...
	@ResponseBody
	public Object selectPage(PageInfo pageInfo, SysConfig sysConfig){
		try {
			Map<String, Object> params = BeanUtils.describe(sysConfig);
			Pager<SysConfig> pager = sysConfigService.findPage(pageInfo, params);
			return pager;
		} catch (Exception e) {
			logger.error("查询出错",e);
		}
	...
	public String preEdit(String id, ModelMap modelMap){
		...
		modelMap.addAttribute("sysConfig", sysConfig);
		return FOLDER + "/edit";
	}
	...
	public Object save(SysConfig sysConfig){
		...
		try {
			if(StringUtils.isEmpty(sysConfig.getId())){
				sysConfig.setId(UUIDUtil.getUUID());
